...
\begin{document}
	...
	W celu zasilenia urządzenia oraz komunikacji zastosowany został zewnętrzny interfejs w formie złącza krawędziowego. Składaja się on z 16 wyjść sterujących, 17 informujacych o stanie maszyny oraz 11 zasilających. W sumie daje to 44 wyjścia. Wszystkie wspomniane sygnały (po za \texttt{Vcc} oraz \texttt{Vss}) zostały wyjaśnione w poprzednim podrozdziale. Wyjścia \texttt{Vcc} oraz \texttt{Vss} są wyjściami odpowiednio zasilania i uziemienia. \texttt{Vcc} powinno mieć napięcie +5V względem \texttt{Vss}. \\
	...
	Oprócz interfejsu zewnętrznego na urządzeniu znajdują się dwa dodatkowe złącza: J2 i J3. Służą one do odczytu wartości odpowiednio \texttt{TAPE SYMBOL REGISTER} oraz \texttt{STATE REGISTER} (złącza te są podłączone do tych modułów bezpośrednio).
