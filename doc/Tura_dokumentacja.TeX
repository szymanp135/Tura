...
\begin{document}
	...
	Maszyna została podzielona na 11 modułów w celu oznakowania jednostek odpowiedzialnych za poszczególne funkcje. Moduły w schematach KiCad zostały oznakowane niebieskimi ramkami i podpisane niebieskim tekstem lub jedynie podpisane niebieskim tekstem w przypadku modułów jednoelementowych.
	\begin{description}
		\item[\texttt{TAPE}] -- Blok pamięci symulujący taśmę. Każda komórka pamięci reprezentuje komórkę taśmy maszyny. Symbol taśmy składa się z 7 bitów danych pamięci (\texttt{D\textsubscript{0...6}});
		...
		\item[\texttt{IN-PROGRAM CONTROL LINES BUFFERS}] --
		\item[\texttt{IN-OPERATION CONTROL LINES BUFFER}] --
	\end{description}
	...
	W celu zasilenia urządzenia oraz komunikacji zastosowany został zewnętrzny interfejs składający się z 20 wyjść sterujących, 17 informujacych o stanie maszyny oraz 7 zasilających. W sumie daje to 44 wyjścia. Wszystkie wspomniane sygnały (po za \texttt{Vcc} oraz \texttt{Vss}) zostały wyjaśnione w poprzednim podrozdziale. By uniknąć zbyt podobnych oznaczeń, kierunki sygnałów zamiast ,,Wej/Wyj'' zostały oznaczone angielskimi ,,In/Out''.
	\newline \newline
	...
	Wyjścia \texttt{Vcc} oraz \texttt{Vss} są wyjściami zasilania. \texttt{Vcc} powinno mieć napięcie +5V względem \texttt{Vss}, które jest uziemieniem.
